...
同一个包两个入口，**选择入口即完成裁剪**——高亮/运行时依赖（yace、es-toolkit、Prism）均预打包进产物（`sideEffects: false`），消费方 bundler 无需做模块级摇树；仅**共享语义包 `@andares/pdfoundation` 保持 external**——两个入口都 re-export `format` / `jsonToPdText` / `pdToJsonText`（另含自研 `highlightPd`），`@andares/pdfoundation` 作为 **dependencies** 自动安装，语义单一来源、零漂移：
...
- 消费验证：vite 项目引 `/pd` 入口构建产物 ~13 kB（gzip ~4.7 kB）、无 Prism 痕迹；除 dependencies `@andares/pdfoundation` 外的依赖均已内联进 dist（仅开发期使用）
