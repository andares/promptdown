...
description: >-
  ...
  触发词：//!pd、promptdown、.pd 文件、pd 格式、pdtransform、pd2json（旧名）。
...
- CLI：`pdtransform <file> [段名|序号]`（自动识别 pd/json 双向转换；多段按段名或 1-based 序号选段）
- 规则：解析行类型 → 块栈按缩进找爸爸 → 键值单条折叠 → Info/Subject 编号 → 引用内联展开
- JSON→pd 反向（`pdtransform 文件.json`）：顶层带子域键值后空一行；`SubjectN` 匿名块还原为裸文本/代码块（前补 `---`）；键形内容项把第一个冒号后跟空格转义为 `:-`；标量转文本、结构性条目丢弃（逐条黄字警告）
