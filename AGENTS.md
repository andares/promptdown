...
- **本项目使用 `npm` 作为唯一包管理器。**
- 一律使用 npm 形式：
  - `npm install` / `npm install -D <pkg>` / `npm run <script>`
  - `npm publish`（由 `npm run release` 自动执行，不要手动跑）
  - `npx <pkg>`（不要用 pnpm）
- 发布脚本：`npm run release <patch|minor|major> [--dry-run]`（参考 `scripts/publish.mjs`）
...
| `npm test` | node:test 跑 `test/*.test.ts`（tsx 执行） |
| `npm run build` | tsc 编译到 `dist/` |
| `npm run package` | `vsce package` 生成 .vsix |
| `npm run release <patch\|minor\|major>` | 一键发布：校验 → 测试 → bump → commit+tag → npm publish → vsce package |
...
6. `npm publish --no-git-checks`（prepublishOnly 再次门禁 typecheck+test+build）
7. `npx vsce package` 生成 `prompt-down-<version>.vsix`
